# -*- coding: utf-8 -*-
'''
Generic functions used across many types of plot,
some of which use the StandardMap object (or similar).

These are mostly going to be handy when making new plotting functions,
and when making multi-panel plots.


These functions don't test that the plot settings 
are actually from a StandardMap object - 
they just need to have the right components within the object.


--------------------------------------------------------
Contents:

* start_figure() sets up a figure object
* start_standard_figure() is a wrapper to start_figure(), 
                          taking its settings from a StandardMap object.


* end_figure() finishes a figure, plotting it or displaying it.

                          
* make_colourbar() creates a colour bar Axes for maps.
* make_standard_bar() is a wrapper using settings from a StandardMap object.


* set_standard_margins() applies the margins from a StandardMap object.
--------------------------------------------------------
...
#===============================================================
def start_figure(cmsize,dpi,fontfam,fsize, figbackgroundcol=None):
    '''
    ...
    # sorted(set([f.name for f in matplotlib.font_manager.fontManager.afmlist+matplotlib.font_manager.fontManager.ttflist]))
...
#===============================================================
def end_figure(outfnames, oldfont_family=None,oldfont_size=None):
    '''
    ...
    if isinstance(outfnames,str): outfnames = [outfnames]
    showit = False
    for outf in outfnames:
        ...
        if extn=="x11":
            ...
            plt.savefig(outf, dpi=plt.gcf().dpi,
                        facecolor=plt.gcf().get_facecolor(), edgecolor='none')
            print "Plot saved to "+outf
    
    if showit:  plt.show() # Have to do this last, it clears the Figure
...
#===============================================================
def set_standard_margins(settings=None,fig=None):
    '''
    Convence wrapper to set margins from the 'settings' object
    (typically a StandardMap object, or similar),
    or just use tight_layout if settings is None.

    The Figure object is optionally provided,
    and obtained from plt.gcf() if not.
    '''
    if settings is None:
        plt.tight_layout()
    else:
        if fig is None: fig = plt.gcf()
        fig.subplots_adjust(left  = settings.marlft,  bottom = settings.marbot,
                            right = settings.marrgt,  top    = settings.martop,
                            wspace= settings.marwsp,  hspace = settings.marhsp )
    return
#===============================================================
...
if __name__=="__main__":
    ...
    '''
    print "====================================================================================="
    print "You have tried to run   ukcp_standard_plots.plotting_general   from the command line."
