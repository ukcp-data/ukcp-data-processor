# -*- coding: utf-8 -*-
'''
Functions for making standard maps for the UKCP18 Climate Service.
...
plot_standard_map(thecube,settings,...):
            This is the main function that Sensible People will use.
            It takes a single 2-D cube (thecube),
            with all the rest of the plot settings wrapped up
            in a  ukcp_standard_plots.standards_class.StandardMap  object.
            
            The idea is that you prepare the data, 
            define/choose/modify a StandardMap object, 
            and then just call this.



plot_map(): A very general map-plotting function.
            This has a lot of possible arguments.
            Have an explore of the different options!



test_stdmap(): A basic test-function. 
               You can specify a StandardMap object for the settings.

plot_choropleth_map()`: similar to plot_map(), 
                        but produces a choropleth (values-in-polygons) map,
                        rather than a map of gridded data (pixels/contours).
...
----------------------------------------
NOTE: For a VERY simple map (not using this system), just say
...
NOT YET IMPLEMENTED:
- Specifying resolution of features
'''
#=========================================================================
import numpy as np
import matplotlib
...
def plot_map(dcube_input, fig=None,ax=None,
             ...
             cpal="RdBu_r",bar_orientation='horizontal',bar_position=None,
             extendcolbar="both", badcol=(1,1,1,0), undercol="magenta",overcol="yellow",
             cmsize=[23,18], dpi=94, fsize=12, fontfam="Arial",
             proj=ccrs.PlateCarree(),
             marlft=0.03, marrgt=0.97, martop=0.96, marbot=0.06, marwsp=0,marhsp=0,
             contlines=None, contlinew=1.0,contlinecol="yellow",contlinealpha=1,
             ...
             dxgrid=20,dygrid=20, xgridax=[True,False], ygridax=[True,False],
    ...
    (based on http://fcm9/projects/ClimateImpacts/browser/pbett/pbvericlimpred/pbskill_maps.py?rev=4935)
    ...
    if preferred_unit is not None:
        if dcube_input.units != preferred_unit:
            print "NOTE: converting cube units (in a copy) from "+dcube_input.units.name + \
                " to "+preferred_unit.name + "..."
            dcube = dcube_input.copy()

            # Got a function to do all this now:
            common.rectify_units(dcube, target_unit=preferred_unit)
    ...
    print "Input cube value range: ",dcube.data.min(), dcube.data.max(),dcube.units.title("")
    ##---------------------------------------------------------------
    ...
    if mask_sea or mask_land:
        if lsm_cube is None:
            print "No land--sea mask specified, " + \
                "but mask requested (sea:",str(mask_sea),"; land:",str(mask_land),",)"
        ...
        # More fool them.
        if mask_sea:
            dcube_masked = common.add_mask(dcube,lsm_cube, comparator="<", threshold=0.5)
        if mask_land:
            dcube_masked = common.add_mask(dcube,lsm_cube, comparator=">", threshold=0.5)    
    else:
        # Leave as it is:
    ...
    if fig is None:
        ...
        newfig = False
        size_inches = None # could probably get this if we wanted.
        print "Figure object already exists, "
        print "Not using plot_map() arguments: "
        print "              cmsize,dpi, fsize,fontfam,"
        print "              marlft,marrgt,martop,marbot,marwsp,marhsp"
    #------------------------------------------------------------
    ...
    if ax is None:
        ...
        newaxes = False
        print "Axes object already exists,"
        print "Not using plot_map() argument: proj"
    ...
    if vmid is None:
        ...
        # Based on a colourbar that extends an equal distance either side of the midpoint,
        # but is then cropped to the value range selected.
    ...
    if cont:
        ...
        cf = iplt.contourf(dcube_masked, levels, cmap=cmap, extend='both')
    else:
        # Shaded gridcells
    ...
    if contlines is not None:
        ...
        cl = iplt.contour(dcube_masked, contlines, linewidths=contlinew, 
    ...
    if regionlcol is not None:
        ...
        vhires_regions = cartopy.feature.NaturalEarthFeature('cultural',
    ...
    if showglobal:
        ...
        xlims_for_grid = xlims if xlims is not None  else [dcube_masked.coord('longitude').points.min(),
                                                           dcube_masked.coord('longitude').points.max() ]
        ylims_for_grid = ylims if ylims is not None  else [dcube_masked.coord( 'latitude').points.min(),
                                                           dcube_masked.coord( 'latitude').points.max() ]
    ...
    if bar_orientation.lower()=="none":
        ...
        plotgeneral.make_colourbar(fig, bar_orientation, extendcolbar, levels,
    ...
    print "All done in plot_map, returning..."
...
def plot_standard_map(thecube,settings, fig=None,ax=None,
    ...
    result = plot_map(thecube, fig=fig,ax=ax, cont=settings.cont,
                      ...
                      contlines=settings.contlines,contlinealpha=settings.contlinealpha,
                      contlinew=settings.contlinew,contlinecol=settings.contlinecol,
                      ...
                      mask_sea=mask_sea, mask_land=mask_land, lsm_cube=lsm_cube, 
...
#=========================================================================
def test_stdmap( settings=stds.UKCPBASIC ):
    '''
    Testing function for the main mapping routine.

    Try this out with different StandardMap objects for the settings,
    like stds.UKCPNEAT or stds.UKCP_TEMP.

    This requires a file in stdplots.TEST_MAPDATA_FILENAME 
    to be present.
    '''
    import iris
    ...
    infile = stdplots.TEST_MAPDATA_FILENAME
    d = iris.load_cube(infile)
    # This is a single global cube, with dims (latitude: 324; longitude: 432)

    # Pull out region:
    reg = regs.REG_BI_DATA
    dhere = regs.reg_extract(d,reg_dict=reg)

    outfnamebase = stdplots.MAP_PLOT_DIR+'testplot_ukcp_standard_plots_mapper'
    outdevs = ["x11","png","pdf"]
    outfnames = [outfnamebase+'.'+dev for dev in outdevs ]

    ax = plot_standard_map(dhere,settings,barlab="Temperature/K",outfnames=outfnames )
...
#=========================================================================
def plot_choropleth_map(regions,regionaldata_input, regional_sigs=None, fig=None,ax=None,
    ...
    based on general_choropleth() in http://fcm9/projects/ClimateImpacts/browser/pbett/ECEM/trunk/plotting_common.py?rev=7165#L131
    ...
    if preferred_unit is not None:
        ...
        for regcube_input in regionaldata_input:
            if regcube_input.units != preferred_unit:
                print "NOTE: converting cube units (in a copy) from " + \
                    regcube_input.units.name + " to "+preferred_unit.name + "..."
                regcube = regcube_input.copy()

                # Got a function to do all this now:
                common.rectify_units(regcube, target_unit=preferred_unit)
    ...
    if regional_sigs is not None:
        raise UserWarning("Not yet implemented how we're handling data for significance shading!")


    # Show the value range, and the units:
    print "Input cube value range: ", \
    ...
    if fig is None:
        ...
        size_inches = None # could probably get this if we wanted.
        print "Figure object already exists, "
        print "Not using plot_choropleth_map() arguments: "
        print "              cmsize,dpi, fsize,fontfam,"
        print "              marlft,marrgt,martop,marbot,marwsp,marhsp"
    #------------------------------------------------------------
    ...
    if ax is None:
        ...
        newaxes = False
        print "Axes object already exists,"
        print "Not using plot_choropleth_map() argument: proj"
    ...
    if do_categories:
        # Categorical case:
        print "Categorical plot selected."
        ...
        # This means that NaNs/unders/overs are replaced with the min/max in the range,
        # so it's useful to add extra categories for these too.
        ...
        # (https://docs.scipy.org/doc/numpy-1.11.0/reference/generated/numpy.arange.html)
        # so this is better:
        nsteps=(vrange[1]-vrange[0])/vstep
        levels = np.linspace(-0.1, 1.0, num=nsteps+1, endpoint=True)
        #print "LEVELS: ",levels
    ...
    for i, region in enumerate(regions):
        ...
        #
        # Output to terminal:
        #print "Region ",i,": "+region.tag+" '"+region.name+"'.   Value = ",val,"("+sigtag+") colour =",facecolor
        print "Region ",i,": '"+reg_name+"'.   Value = ",val,", colour =",facecolor
    ...
    if othershapefile is not None:
        ...
        for i,r in enumerate(selshapes):
            ...
            if reg_proj  != proj:
                selshapes[i].geometry  = proj.project_geometry(r.geometry, src_crs=reg_proj )

        # Plot:
        ax.add_geometries([shp.geometry for shp in selshapes], ccrs.PlateCarree(),
                          edgecolor=subregionlcol,facecolor="", linewidth=subregionlw)
    ...
    if bar_orientation.lower()=="none":
        ...
        plotgeneral.make_colourbar(fig, bar_orientation, extendcolbar, levels,
                                   barlabel, colmappable=sm, bar_pos=bar_position,
                                   ticklen=ticklen, ticklabs=ticklabs )    
    ##-------------------------------------------------------------------
...
if __name__=="__main__":
    ...
    with a choice of StandardMap object.
    '''
    test_stdmap(settings=stds.UKCPNEAT)
