"""
This module contains the SingleMapShpWriter class, which implements the _write_shp
method from the BaseShpWriter base class.
...
class SingleMapShpWriter(BaseShpWriter):
    """
    The single map shapefile writer class.
    ...
    def _write_shp(self):
        """
        Write out the data, in shapefile format, associated with a single map.
    ...
    def _write_bbox_shp(self):
        ...
        output_data_file = self._get_file_name()

        self._write_bbox_data(
            cube, half_grid_size, output_data_file, output_file_list, var_label
        )
