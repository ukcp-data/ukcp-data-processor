...
def test_make_anomaly_percentage_change():
    u = Unit("%")
    c = iris.cube.Cube([15.0], long_name="rain", units="%")
    ref = iris.cube.Cube([10.0], long_name="rain", units="%")
...
def test_make_anomaly_absolute_change():
    u = Unit("Celsius")
    c = iris.cube.Cube([15.0], long_name="temp", units="degC")
    ref = iris.cube.Cube([10.0], long_name="temp", units="degC")
    anom = _make_anomaly(c, ref, u)
