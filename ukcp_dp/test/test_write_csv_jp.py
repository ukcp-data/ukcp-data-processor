...
def get_ls1_test_prob_point_data():
    ...
    reference_files = [
        path.join(base_path, "data", "expected_outputs", "LS1_JP_01_point_seasonal.csv")
    ]
...
class LS1JPCsvTestCase(unittest.TestCase):
    def test_ls1_point_csv(self):
        data, input_files, reference_files, output_file_index = (
            get_ls1_test_prob_point_data()
        )
        diff = run_write_test(
            data, input_files, reference_files, output_file_index, PlotType.JP_PLOT
        )
