...
class JpPlotter(GraphPlotter):
    ...
    def _generate_graph(self):
        ...
        for cube in self.cube_list:
            if cube.var_name == self.input_data.get_value(InputType.VARIABLE)[0]:
                x = cube.data
                x_id = self.input_data.get_value(InputType.VARIABLE)[0]
            elif cube.var_name == self.input_data.get_value(InputType.VARIABLE)[1]:
                y = cube.data
        ...
        contour_fill = plt.contourf(
            xbins, ybins, new_h, levels, colors=CONTOUR_FILL, extend="max", label=levels
        )
        ...
        if (x_id == "tasAnom" or y_id == "tasAnom") and (
            ...
            at = AnchoredText(
                "hotter and wetter",
                prop=dict(color="#612020", size=self.input_data.get_font_size()),
                frameon=False,
                loc=1,
            )
            ax.add_artist(at)
            ...
            at = AnchoredText(
                "colder and drier",
                prop=dict(color="#2F7676", size=self.input_data.get_font_size()),
                frameon=False,
                loc=3,
            )
            ax.add_artist(at)
